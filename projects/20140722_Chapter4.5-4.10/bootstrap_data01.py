...
#------------------------------------------------------------
# Compute the theoretical expectations for the two distributions; s=1.0
x = np.linspace(0.8, 1.2, num=m)     ## len(x) = m

sigma1 = 1. / np.sqrt(2 * (m - 1))  ## Standard error of mean, eq 3.35
pdf1 = norm(1, sigma1).pdf(x)       ## PDF of norm dist at x; length = m

sigma2 = 1.06 / np.sqrt(m)    ## Sigma from inner 50% quantile; eq. 3.36-3.38
